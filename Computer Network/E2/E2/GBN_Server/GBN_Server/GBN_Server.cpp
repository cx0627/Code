...
//|1 （1个字节）|nextSendSeq （1个字节）|ack确认seq （1个字节）|data （1024个字节）
int windowSize;//发送窗口大小,停等协议为1，GBN协议为8
			...
			while (flag) {//发送205
				if (state == 0) {
					...
					if (seqIsAvailable() && (totalSeq + 1 <= totalPacket)) {
						...
						cout << "成功发送序列号为:" << nextSendSeq << "的分组发送ACK."<<r_seq << endl;
					...
					if (recvSize < 0) {//没收到ack
						...
						r_seq = buffer[2];
						if (r != -1) {
							cout << "收到ack" << r  <<",收到序列号"<<r_seq<<"的分组"<< endl;
							if (r < base) {

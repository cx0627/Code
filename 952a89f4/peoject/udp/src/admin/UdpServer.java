...
public class UdpServer extends JFrame implements ActionListener{
	...
	public UdpServer(int nowsendPort,String nowip,int nowrecPort,Server getConnectClient,String talkName) {
		...
		this.setTitle(name.getText()+" 的服务器会话窗口");
		...
		if(serverTextThread==null){
			...
			System.out.println("已经启动服务器，接收端口号是 ："+recPort+"\n");
		...
		if(e.getSource()==send){
			...
			if(!message.equals("")){
				try{
					...
					ds = null;
					dp = null;
					System.out.println("发送信息到客户端： "+sendPort+" 消息  "+mess);
					area.append(mess+"\n");
					areaSend.setText("");
					setMouse();
				}catch(Exception ee){   
	...
	public static void main(String[] args) {
		...
		new UdpServer(11,"localhost",22,new Server(),"服务器") ;
	}
